...
if [ "$answer" = "y" -o "$answer" = "Y" ] ; then
	...
	if [ $? = 0 ] ; then
		...
		if [ $? = 0 ] ; then
				...
				update_pip3=$(sudo -H apt install python3-pip && sudo -H pip3 install --upgrade pip)&
				wait $update_pip3
				printf "\nInstalling jupyter...\n"
			...
			if [ $(pgrep jupyter) ] ; then
				...
				printf "Process lifetime: 20s\n"
				$(timeout 20s jupyter notebook --NotebookApp.allow_origin='https://colab.research.google.com' --port=8888 --NotebookApp.port_retries=0 >> $LOG_FILE)
